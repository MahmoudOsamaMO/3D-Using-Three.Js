...
app.controller('RoofPlatformsCtrl', function ($scope, $http,$log) {
    ...
    $scope.removeAcc = function (index) {
        ...
        $scope.Acchistory.push($scope.building.Subsystems.RoofPlatforms.RoofPlatforms[index]);
        // Remove from main records (using index)
        $scope.building.Subsystems.RoofPlatforms.RoofPlatforms.splice(index, 1);
    };
    // Undo action (delete)
    $scope.UndoAcc = function () {
        $scope.building.Subsystems.RoofPlatforms.RoofPlatforms.push($scope.Acchistory[$scope.Acchistory.length - 1]);
    ...
    $scope.ResetAcc = function () {
        $scope.Accword = "Add";
        var id = 1;
        if ($scope.building.Subsystems.RoofPlatforms.RoofPlatforms)
            id = $scope.building.Subsystems.RoofPlatforms.RoofPlatforms.length + 1;
        ...
        $scope.newobj = {
            ID: id,
            "Width": { "Width": 54, "FromGL": WidthFromGL, "ToGL": WidthFromGL },
            "Length": { "Length": 54, "FromGL": LengthFromGL, "ToGL": LengthFromGL },
            "Area": 2916,
            "BottomHeightFromFFL": 45,
            "Flooring": { "By": "ZS", "Type": "CHECK. PLATE", "Thickness": 345 },
            "Load": { "UniformLoad": 456, "ConcentratedLoad": 432 },
            "HandrailLength": 666
        };
    };
    //Add  Bracing 
    $scope.AddAcc = function () {
        if ($scope.building.Subsystems.RoofPlatforms.RoofPlatforms)
            $scope.building.Subsystems.RoofPlatforms.RoofPlatforms.push(angular.copy($scope.newobj));
        else
            $scope.building.Subsystems.RoofPlatforms.RoofPlatforms = [angular.copy($scope.newobj)];
    };
    $scope.AccUpdate = function (index) {
        $scope.newobj = angular.copy($scope.building.Subsystems.RoofPlatforms.RoofPlatforms[index]);
    ...
    $scope.ModyfyAcc = function () {
        $scope.building.Subsystems.RoofPlatforms.RoofPlatforms[$scope.accIndex] = angular.copy($scope.newobj);
    };
    ...
    $scope.removeRemark = function (index) {
        ...
        $scope.building.Subsystems.RoofPlatforms.Remarks.Remark[index].State = "false";
    };
    // Undo action (delete)
    $scope.UndoRemark = function () {
        $scope.building.Subsystems.RoofPlatforms.Remarks.Remark.push($scope.Remarkhistory[$scope.Remarkhistory.length - 1]);
    ...
    $scope.ResetRemark = function () {
        $scope.Remarkword = "Add";
        var id = 1;
        if ($scope.building.Subsystems.RoofPlatforms.Remarks.Remark)
            id = $scope.building.Subsystems.RoofPlatforms.Remarks.Remark.length + 1;
        $scope.newRemark = {
            ID: id,
            Remark: "",
            State: "true"
    ...
    $scope.AddRemark = function () {
        if ($scope.building.Subsystems.RoofPlatforms.Remarks.Remark)
            $scope.building.Subsystems.RoofPlatforms.Remarks.Remark.push(angular.copy($scope.newRemark));
        else
            $scope.building.Subsystems.RoofPlatforms.Remarks.Remark = [angular.copy($scope.newRemark)];
    };
    $scope.RemarkUpdate = function (index) {
        $scope.newRemark = angular.copy($scope.building.Subsystems.RoofPlatforms.Remarks.Remark[index]);
    ...
    $scope.ModyfyRemark = function () {
        $scope.building.Subsystems.RoofPlatforms.Remarks.Remark[$scope.RemarkIndex] = angular.copy($scope.newRemark);
    };
