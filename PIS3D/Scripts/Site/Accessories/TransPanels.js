...
app.controller('TranslucentPanelsCtrl', function ($scope, $http,$log) {
    ...
    $scope.AccName = "Translucent Panel";
    ...
    $scope.removeAcc = function (index) {
        ...
        $scope.Acchistory.push($scope.building.Openings.TranslucentPanels.TranslucentPanel[index]);
        // Remove from main records (using index)
        $scope.building.Openings.TranslucentPanels.TranslucentPanel.splice(index, 1);
    };
    // Undo action (delete)
    $scope.UndoAcc = function () {
        $scope.building.Openings.TranslucentPanels.TranslucentPanel.push($scope.Acchistory[$scope.Acchistory.length - 1]);
    ...
    $scope.ResetAcc = function () {
        $scope.Accword = "Add";
        var id = 1;
        if ($scope.building.Openings.TranslucentPanels.TranslucentPanel)
            id = $scope.building.Openings.TranslucentPanels.TranslucentPanel.length + 1;
        $scope.newobj = {
            AssignTo: "NSW",
            Height: 0,
            ID: id,
    ...
    $scope.AddAcc = function () {
        if ($scope.building.Openings.TranslucentPanels.TranslucentPanel)
            $scope.building.Openings.TranslucentPanels.TranslucentPanel.push($scope.newobj);
        else
            $scope.building.Openings.TranslucentPanels.TranslucentPanel = [$scope.newobj];
    };
    $scope.AccUpdate = function (index) {
        $scope.newobj = angular.copy($scope.building.Openings.TranslucentPanels.TranslucentPanel[index]);
        $scope.accIndex = index;
        $('#AccModal').modal('show');
        $scope.Accword = "Update";
        if ($scope.newobj.Material)
            $scope.showadv = true;
    };
    ...
    $scope.ModyfyAcc = function () {
        $scope.building.Openings.TranslucentPanels.TranslucentPanel[$scope.accIndex] = angular.copy($scope.newobj);
    };
    ...
    $scope.Materials = ["FRP", "Polycarbonate", "NONE"];
    ...
    $scope.removeRemark = function (index) {
        ...
        $scope.building.Openings.TranslucentPanels.Remarks.Remark[index].State = "false";
    };
    // Undo action (delete)
    $scope.UndoRemark = function () {
        $scope.building.Openings.TranslucentPanels.Remarks.Remark.push($scope.Remarkhistory[$scope.Remarkhistory.length - 1]);
    ...
    $scope.ResetRemark = function () {
        $scope.Remarkword = "Add";
        var id = 1;
        if ($scope.building.Openings.TranslucentPanels.Remarks.Remark)
            id = $scope.building.Openings.TranslucentPanels.Remarks.Remark.length + 1;
        $scope.newRemark = {
            ID: id,
            Remark: "",
            State: "true"
    ...
    $scope.AddRemark = function () {
        if ($scope.building.Openings.TranslucentPanels.Remarks.Remark)
            $scope.building.Openings.TranslucentPanels.Remarks.Remark.push(angular.copy($scope.newRemark));
        else
            $scope.building.Openings.TranslucentPanels.Remarks.Remark = [angular.copy($scope.newRemark)];
    };
    $scope.RemarkUpdate = function (index) {
        $scope.newRemark = angular.copy($scope.building.Openings.TranslucentPanels.Remarks.Remark[index]);
    ...
    $scope.ModyfyRemark = function () {
        $scope.building.Openings.TranslucentPanels.Remarks.Remark[$scope.RemarkIndex] = angular.copy($scope.newRemark);
    };
