...
app.controller('FramedOpiningsCtrl', function ($scope, $http,$log) {
    ...
    $scope.removeAcc = function (index) {
        ...
        $scope.Acchistory.push($scope.building.Openings.FramedOpenings.FramedOpening[index]);
        // Remove from main records (using index)
        $scope.building.Openings.FramedOpenings.FramedOpening.splice(index, 1);
    };
    // Undo action (delete)
    $scope.UndoAcc = function () {
        $scope.building.Openings.FramedOpenings.FramedOpening.push($scope.Acchistory[$scope.Acchistory.length - 1]);
    ...
    $scope.ResetAcc = function () {
        $scope.Accword = "Add";
        var id = 1;
        if ($scope.building.Openings.FramedOpenings.FramedOpening)
            id = $scope.building.Openings.FramedOpenings.FramedOpening.length + 1;
        $scope.newobj = {
            AssignTo: "NSW",
            CarryingWeight: 0,
            Height: 0,
    ...
    $scope.AddAcc = function () {
        if ($scope.building.Openings.FramedOpenings.FramedOpening)
            $scope.building.Openings.FramedOpenings.FramedOpening.push(angular.copy($scope.newobj));
        else
            $scope.building.Openings.FramedOpenings.FramedOpening = [angular.copy($scope.newobj)];
    };
    $scope.AccUpdate = function (index) {
        $scope.newobj = angular.copy($scope.building.Openings.FramedOpenings.FramedOpening[index]);
    ...
    $scope.ModyfyAcc = function () {
        $scope.building.Openings.FramedOpenings.FramedOpening[$scope.accIndex] = angular.copy($scope.newobj);
    };
    ...
    $scope.AdvancedFunc = function () {
        if ($scope.newobj.AssignTo == "Roof") {
            $scope.Purpuses = $scope.PurpusesRoof;
            $scope.newobj.Purpose = "POWER VENT.";
        }
        else {
            $scope.Purpuses = $scope.PurpusesElse;
            $scope.newobj.Purpose = "WINDOW";
        }
    };
    $scope.Purpuses = ["WINDOW", "LOUVER", "ROLL-UP DOOR", "SLIDING DOOR", "PERSONNAL DOOR", "AC UNIT", "AC DUCT", "EXHAUST FAN", "OTHER"];
    $scope.PurpusesRoof = ["POWER VENT.", "AC DUCT", "CHIMNEY"];
    $scope.PurpusesElse = ["WINDOW", "LOUVER", "ROLL-UP DOOR", "SLIDING DOOR", "PERSONNAL DOOR", "AC UNIT", "AC DUCT", "EXHAUST FAN", "OTHER"];
    ...
    $scope.removeRemark = function (index) {
        ...
        $scope.building.Openings.FramedOpenings.Remarks.Remark[index].State = "false";
    };
    // Undo action (delete)
    $scope.UndoRemark = function () {
        $scope.building.Openings.FramedOpenings.Remarks.Remark.push($scope.Remarkhistory[$scope.Remarkhistory.length - 1]);
    ...
    $scope.ResetRemark = function () {
        $scope.Remarkword = "Add";
        var id = 1;
        if ($scope.building.Openings.FramedOpenings.Remarks.Remark)
            id = $scope.building.Openings.FramedOpenings.Remarks.Remark.length + 1;
        $scope.newRemark = {
            ID: id,
            Remark: "",
            State: "true"
    ...
    $scope.AddRemark = function () {
        if ($scope.building.Openings.FramedOpenings.Remarks.Remark)
            $scope.building.Openings.FramedOpenings.Remarks.Remark.push(angular.copy($scope.newRemark));
        else
            $scope.building.Openings.FramedOpenings.Remarks.Remark = [angular.copy($scope.newRemark)];
    };
    $scope.RemarkUpdate = function (index) {
        $scope.newRemark = angular.copy($scope.building.Openings.FramedOpenings.Remarks.Remark[index]);
    ...
    $scope.ModyfyRemark = function () {
        $scope.building.Openings.FramedOpenings.Remarks.Remark[$scope.RemarkIndex] = angular.copy($scope.newRemark);
    };
