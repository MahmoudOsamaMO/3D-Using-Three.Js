...
app.controller('OpenWallsCtrl', function ($scope, $http,$log) {
    ...
    $scope.removeAcc = function (index) {
        ...
        $scope.Acchistory.push($scope.building.Openings.OpenWalls.OpenWall[index]);
        // Remove from main records (using index)
        $scope.building.Openings.OpenWalls.OpenWall.splice(index, 1);
    };
    // Undo action (delete)
    $scope.UndoAcc = function () {
        $scope.building.Openings.OpenWalls.OpenWall.push($scope.Acchistory[$scope.Acchistory.length - 1]);
    ...
    $scope.ResetAcc = function () {
        $scope.Accword = "Add";
        var id = 1;
        if ($scope.building.Openings.OpenWalls.OpenWall)
            id = $scope.building.Openings.OpenWalls.OpenWall.length + 1;
        $scope.newobj = {
            AssignTo: "NSW",
            Height: 0,
            FullLength:false,
    ...
    $scope.AddAcc = function () {
        if ($scope.building.Openings.OpenWalls.OpenWall)
            $scope.building.Openings.OpenWalls.OpenWall.push(angular.copy($scope.newobj));
        else
            $scope.building.Openings.OpenWalls.OpenWall = [angular.copy($scope.newobj)];
    };
    $scope.AccUpdate = function (index) {
        $scope.newobj = angular.copy($scope.building.Openings.OpenWalls.OpenWall[index]);
        $scope.accIndex = index;
        $('#AccModal').modal('show');
        $scope.Accword = "Update";
        if ($scope.newobj.OpenFor || $scope.newobj.Thickness)
            $scope.showadv = true;
    };
    ...
    $scope.ModyfyAcc = function () {
        $scope.building.Openings.OpenWalls.OpenWall[$scope.accIndex] = angular.copy($scope.newobj);
    };
    ...
    $scope.OpenFors = ["BLOCK WALL", "ACCESS", "PRECAST PANEL", "GLAZING", "NONE"];
    $scope.Fulllength = function () {
        if ($scope.newobj.FullLength == true) {
            if ($scope.newobj.AssignTo == "REW" || $scope.newobj.AssignTo == "LEW")
                $scope.newobj.Width = $scope.building.GeneralData.Width;
            else
                $scope.newobj.Width = $scope.building.GeneralData.Length;
            //$scope.newobj.ReferenceTo = ""
            $scope.myVar = false
        }
    }
    ...
    $scope.removeRemark = function (index) {
        ...
        $scope.building.Openings.OpenWalls.Remarks.Remark[index].State = "false";
    };
    // Undo action (delete)
    $scope.UndoRemark = function () {
        $scope.building.Openings.OpenWalls.Remarks.Remark.push($scope.Remarkhistory[$scope.Remarkhistory.length - 1]);
    ...
    $scope.ResetRemark = function () {
        $scope.Remarkword = "Add";
        var id = 1;
        if ($scope.building.Openings.OpenWalls.Remarks.Remark)
            id = $scope.building.Openings.OpenWalls.Remarks.Remark.length + 1;
        $scope.newRemark = {
            ID: id,
            Remark: "",
            State: "true"
    ...
    $scope.AddRemark = function () {
        if ($scope.building.Openings.OpenWalls.Remarks.Remark)
            $scope.building.Openings.OpenWalls.Remarks.Remark.push(angular.copy($scope.newRemark));
        else
            $scope.building.Openings.OpenWalls.Remarks.Remark = [angular.copy($scope.newRemark)];
    };
    $scope.RemarkUpdate = function (index) {
        $scope.newRemark = angular.copy($scope.building.Openings.OpenWalls.Remarks.Remark[index]);
    ...
    $scope.ModyfyRemark = function () {
        $scope.building.Openings.OpenWalls.Remarks.Remark[$scope.RemarkIndex] = angular.copy($scope.newRemark);
    };
