...
app.controller('PersonalDoorsCtrl', function ($scope, $http,$log) {
    ...
    $scope.removeAcc = function (index) {
        ...
        $scope.Acchistory.push($scope.building.Openings.PersonnelDoors.PersonnelDoor[index]);
        // Remove from main records (using index)
        $scope.building.Openings.PersonnelDoors.PersonnelDoor.splice(index, 1);
    };
    // Undo action (delete)
    $scope.UndoAcc = function () {
        $scope.building.Openings.PersonnelDoors.PersonnelDoor.push($scope.Acchistory[$scope.Acchistory.length - 1]);
    ...
    $scope.ResetAcc = function () {
        $scope.Accword = "Add";
        var id = 1;
        if ($scope.building.Openings.PersonnelDoors.PersonnelDoor)
            id = $scope.building.Openings.PersonnelDoors.PersonnelDoor.length + 1;
        $scope.newobj = {
            AssignTo: "NSW",
            Height: 2134,
            ID: id,
    ...
    $scope.AddAcc = function () {
        if ($scope.building.Openings.PersonnelDoors.PersonnelDoor)
            $scope.building.Openings.PersonnelDoors.PersonnelDoor.push(angular.copy($scope.newobj));
        else
            $scope.building.Openings.PersonnelDoors.PersonnelDoor = [angular.copy($scope.newobj)];

    };
    $scope.AccUpdate = function (index) {
        $scope.newobj = angular.copy($scope.building.Openings.PersonnelDoors.PersonnelDoor[index]);
        $scope.accIndex = index;
        $('#AccModal').modal('show');
        $scope.Accword = "Update";
        if ($scope.newobj.Hardware || $scope.newobj.Type)
            $scope.showadv = true;
    };
    ...
    $scope.ModyfyAcc = function () {
        $scope.building.Openings.PersonnelDoors.PersonnelDoor[$scope.accIndex] = angular.copy($scope.newobj);
    };
    ...
    $scope.newobjTypes = ["DOUBLE", "SINGLE", "PILOT", "NONE"];
    $scope.newobjHardwares = ["STANDARD", "PANIC", "NONE"];

    $scope.ChangeDoor = function () {
        if ($scope.newobj.Type == "DOUBLE") {
            $scope.newobj.Width = 1829;
            $scope.newobj.Height = 2134;
        }

        if ($scope.newobj.Type == "SINGLE") {
            $scope.newobj.Width = 914;
            $scope.newobj.Height = 2134;
        }
        if ($scope.newobj.Type == "PILOT" || $scope.newobj.Type == "NONE") {
            $scope.newobj.Width = 0;
            $scope.newobj.Height = 0;
        }
    };
    ...
    $scope.removeRemark = function (index) {
        ...
        $scope.building.Openings.PersonnelDoors.Remarks.Remark[index].State = "false";
    };
    // Undo action (delete)
    $scope.UndoRemark = function () {
        $scope.building.Openings.PersonnelDoors.Remarks.Remark.push($scope.Remarkhistory[$scope.Remarkhistory.length - 1]);
    ...
    $scope.ResetRemark = function () {
        $scope.Remarkword = "Add";
        var id = 1;
        if ($scope.building.Openings.PersonnelDoors.Remarks.Remark)
            id = $scope.building.Openings.PersonnelDoors.Remarks.Remark.length + 1;
        $scope.newRemark = {
            ID: id,
            Remark: "",
            State: "true"
    ...
    $scope.AddRemark = function () {
        if ($scope.building.Openings.PersonnelDoors.Remarks.Remark)
            $scope.building.Openings.PersonnelDoors.Remarks.Remark.push(angular.copy($scope.newRemark));
        else
            $scope.building.Openings.PersonnelDoors.Remarks.Remark = [angular.copy($scope.newRemark)];
    };
    $scope.RemarkUpdate = function (index) {
        $scope.newRemark = angular.copy($scope.building.Openings.PersonnelDoors.Remarks.Remark[index]);
    ...
    $scope.ModyfyRemark = function () {
        $scope.building.Openings.PersonnelDoors.Remarks.Remark[$scope.RemarkIndex] = angular.copy($scope.newRemark);
    };
