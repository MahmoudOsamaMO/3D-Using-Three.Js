...
app.controller('WindowsCtrl', function ($scope, $http,$log) {
    ...
    $scope.removeAcc = function (index) {
        ...
        $scope.Acchistory.push($scope.building.Openings.Windows.Window[index]);
        // Remove from main records (using index)
        $scope.building.Openings.Windows.Window.splice(index, 1);
    };
    // Undo action (delete)
    $scope.UndoAcc = function () {
        $scope.building.Openings.Windows.Window.push($scope.Acchistory[$scope.Acchistory.length - 1]);
    ...
    $scope.ResetAcc = function () {
        $scope.Accword = "Add";
        var id = 1;
        if ($scope.building.Openings.Windows.Window)
            id = $scope.building.Openings.Windows.Window.length + 1;
        $scope.newobj = {
            AssignTo: "NSW",
            CarryingWeight: 0,
            Height: 0,
    ...
    $scope.AddAcc = function () {
        if ($scope.building.Openings.Windows.Window)
            $scope.building.Openings.Windows.Window.push($scope.newobj);
        else
            $scope.building.Openings.Windows.Window = [$scope.newobj];
    };
    $scope.AccUpdate = function (index) {
        $scope.newobj = angular.copy($scope.building.Openings.Windows.Window[index]);
    ...
    $scope.ModyfyAcc = function () {
        $scope.building.Openings.Windows.Window[$scope.accIndex] = angular.copy($scope.newobj);
    };
    ...
    $scope.Types = ["STANDARD", "SPECIAL", "NONE"];
    $scope.OperationTypes = ["FIXED", "HALF SLIDING", "FULL SLIDING", "NONE"];
    $scope.LocatedOns = ["STEEL PANEL", "BLOCK WALL", "NONE"];
    ...
    $scope.removeRemark = function (index) {
        ...
        $scope.building.Openings.Windows.Remarks.Remark[index].State = "false";
    };
    // Undo action (delete)
    $scope.UndoRemark = function () {
        $scope.building.Openings.Windows.Remarks.Remark.push($scope.Remarkhistory[$scope.Remarkhistory.length - 1]);
    ...
    $scope.ResetRemark = function () {
        $scope.Remarkword = "Add";
        var id = 1;
        if ($scope.building.Openings.Windows.Remarks.Remark)
            id = $scope.building.Openings.Windows.Remarks.Remark.length + 1;
        $scope.newRemark = {
            ID: id,
            Remark: "",
            State: "true"
    ...
    $scope.AddRemark = function () {
        if ($scope.building.Openings.Windows.Remarks.Remark)
            $scope.building.Openings.Windows.Remarks.Remark.push(angular.copy($scope.newRemark));
        else
            $scope.building.Openings.Windows.Remarks.Remark = [angular.copy($scope.newRemark)];
    };
    $scope.RemarkUpdate = function (index) {
        $scope.newRemark = angular.copy($scope.building.Openings.Windows.Remarks.Remark[index]);
    ...
    $scope.ModyfyRemark = function () {
        $scope.building.Openings.Windows.Remarks.Remark[$scope.RemarkIndex] = angular.copy($scope.newRemark);
    };
